...
cLinkedList * clinkedlist_create(uint32_t(*c_equals)(void * this, void * another),
	...
	if (!p) //分配内存失败
		return NULL;
	cLinkedListMetadata *  metadate_p = (cLinkedListMetadata * )malloc(sizeof(cLinkedListMetadata));
	if (!metadate_p) {
		free(p);
		return NULL;
	}
...
void * clinkedlist_get(cLinkedList * list,uint32_t index) {
	...
	cLinkedListNode * elem = _clinkedlist_find_element_pointer(list,index);
	if (elem)
		return elem->elem;
	else
		return NULL;
